...
#include "convert.h"
#include "mat.h"
#include "FS_elle2fft_onlyunodes.h"
...
int Elle2fft()
{
        ...
        for (c=0;c<cols;c++,n++) {
            ...
            if (i!=NO_VAL) {
               ...
               //  it is not reset in the 2nd write block below
                ...
                // FS: Use unode viscosity as phase indicative
                ElleGetUnodeAttribute(i,&jphase,U_VISCOSITY);
    ...
    if (outf) {
    	...
    	//outf << grain_count << endl; // COMMENTED BY FS
    	//outf << setfill(' ') << setw(10) << setprecision(5); // COMMENTED BY FS
        outf << "0" << endl;
    }
    else OnError((char *)outfilename.c_str(),OPEN_ERR);

    // Write the first block: COMMENTS BY FS: DO NOT WRITE THIS BLOCK
    max = ElleMaxFlynns();
